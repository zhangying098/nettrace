...
static inline fake_analy_ctx_t *analy_fake_ctx_alloc(u64 key, analy_ctx_t *ctx)
{
...
static inline void do_basic_poll(analy_entry_t *entry)
{
	...
	// 根据 event->func 获取当前跟踪函数的结构体（该结构体由 gen_trace.py 生成的 trace_group.c 中）
	trace = get_trace_from_analy_entry(entry);
