...
#define bpf_set_config(skel, sec, value)             \
	do                                               \
	{                                                \
...
#define bpf_set_config_field(skel, sec, name, value) \
	do                                               \
	{                                                \
		int fd = bpf_map__fd(skel->maps.m_config);   \
		u8 buf[CONFIG_MAP_SIZE] = {};                \
...
int perf_output_cond(int fd, perf_buffer_sample_fn cb, perf_buffer_lost_fn lost,
					 bool *stop);
